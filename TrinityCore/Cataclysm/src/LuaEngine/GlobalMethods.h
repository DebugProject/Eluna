...
namespace LuaGlobalFunctions
{
    ...
    int AddVendorItem(lua_State* L)
    {
        ...
        if (!sObjectMgr->IsVendorItemValid(entry, item, maxcount, incrtime, extendedcost, 0)) // @TODO CHANGE TYPE
            return 0;
    ...
    int VendorRemoveItem(lua_State* L)
    {
        ...
        sObjectMgr->RemoveVendorItem(entry, item, true);
        return 0;
    }
    ...
    int VendorRemoveAllItems(lua_State* L)
    {
        ...
        for (VendorItemList::const_iterator itr = itemlist.begin(); itr != itemlist.end(); ++itr)
            sObjectMgr->RemoveVendorItem(entry, (*itr)->item, 0); // @TODO Change compatibility
        return 0;
    }
